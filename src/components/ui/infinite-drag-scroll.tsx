...
export const DraggableContainer = ({
  ...
  return (
    <GridVariantContext.Provider value={variant}>
      <div className="h-[70vh] md:h-dvh overflow-hidden">
        <motion.div
          className="h-[70vh] md:h-dvh overflow-hidden"
        >
          <motion.div
            className={cn(
              "grid h-fit w-fit cursor-grab grid-cols-[repeat(3,1fr)] md:grid-cols-[repeat(2,1fr)] bg-[#141414] active:cursor-grabbing will-change-transform",
              className,
            )}
            drag
            dragMomentum={true}
  ...
  const gridItemStyles = cva(
    "overflow-hidden hover:cursor-pointer w-full h-full will-change-transform",
    {
      variants: {
        variant: {
          default: "rounded-sm",
          masonry: "even:mt-[40%] md:even:mt-[60%] rounded-sm",
          polaroid:
            "border-10 border-b-28 border-white shadow-xl even:rotate-3 odd:-rotate-2 hover:rotate-0 transition-transform ease-out duration-300 even:mt-[40%] md:even:mt-[60%]",
        },
      },
      defaultVariants: {
        variant: "default",
    ...
    const gridBodyStyles = cva("grid grid-cols-[repeat(6,1fr)] h-fit w-fit", {
      variants: {
        variant: {
          ...
          masonry: "gap-x-14 gap-y-8 px-7 md:gap-x-28 md:gap-y-0 md:px-14",
          polaroid: "gap-x-14 gap-y-8 px-7 md:gap-x-28 md:gap-y-0 md:px-14",
        },
      },
      defaultVariants: {
        variant: "default",
