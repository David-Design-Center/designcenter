...
import { Menu, X, Phone, Volume2, VolumeX, ChevronDown } from 'lucide-react';
...
const menuItems: MenuItem[] = [
  ...
  },
  {
    title: 'LOCATIONS',
    href: '/service-areas',
    image: 'https://res.cloudinary.com/designcenter/image/upload/v1767974874/2_lromdm.avif',
    alt: 'Service Areas – D&D Design Center',
    children: [
      { title: 'New York City', href: '/service-areas/new-york-city' },
      { title: 'Long Island', href: '/service-areas/long-island' },
      { title: 'New Jersey', href: '/service-areas/new-jersey' },
      { title: 'Boca Raton, FL', href: '/service-areas/boca-raton-florida' },
      { title: 'Miami, FL', href: '/service-areas/miami-florida' },
    ],
  },
  {
    title: 'PRODUCTS',
  ...
  },
  {
    title: 'DESIGNERS',
    href: '/designers',
    image: 'https://res.cloudinary.com/designcenter/image/upload/Designers.avif',
    alt: 'Meet Our Luxury Interior Designers – D&D Design Center',
  },
  {
    title: 'BLOG',
    href: '/blog',
              ...
              <div className="relative">
                <nav className={`items-start relative z-10 ${
                  ...
                  {menuItems.map((item, index) => {
                    ...
                    if (item.children) {
                      return (
                        <div key={item.title} className={isCompactMode ? 'col-span-2' : ''}>
                          <div className="flex items-center">
                            ...
                            <button
                              ...
                              className="ml-1 p-1 text-[#2D2D2D] hover:text-[#C5A267] transition-all duration-300 min-w-[32px] min-h-[32px] flex items-center justify-center"
                              aria-label={isServicesOpen ? 'Collapse service areas' : 'Expand service areas'}
                            >
                            ...
                            <div className="pl-4 pt-1 pb-2 space-y-0.5">
                              {item.children.map((child) => (
                                <Link
                                  key={child.title}
                                  to={child.href}
                                  className="block text-[18px] md:text-[16px] font-serif text-gray-500 hover:text-[#C5A267] transition-colors duration-200 py-1.5 pl-3 border-l-2 border-gray-200 hover:border-[#C5A267]"
                                  onClick={handleClose}
                                  onMouseEnter={() => {
                                    setActiveItem(index);
                                    setIsHovered(true);
